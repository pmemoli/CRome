/* Regression test for rewriting cvttss2si instructions whose destinations are
 ...
 * register allocation, cvttss2si destination operands in those programs will
 * be allocated to hard registers, not memory.
 *
 * In this program, we include a cvttss2si whose destination will be spilled
...
int main(void) {
    // The cvttss2si operation whose result we want to spill;
    ...
    // create 12 pseudos that all interfere w/ cvttss2si result and each
    ...
    // and cvttss2si result, so cvttss2si result will have more conflicts than
    ...
    check_12_ints(thirteen, fourteen, fifteen, sixteen, seventeen, eighteen,
                  ...
                  twenty_four, 13);
    // use cvttss2si result to make it interfere with other pseudos
